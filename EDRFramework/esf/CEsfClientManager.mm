...
bool CEsfClientManager::Initialize()
{
    try
    {
...
void CEsfClientManager::SetDefaultSubscription()
{
    setNotifySubscription();
    setAuthSubscription();
}
...
bool CEsfClientManager::initializeNotifyClient()
{
    ...
    if ( __builtin_available(macOS 10.15, *) )
    {
        ...
        LOG_INFO("Notify client initialized successfully");
        return true;
    }
    else
    {
...
bool CEsfClientManager::initializeAuthClient()
{
    ...
    if ( __builtin_available(macOS 10.15, *) )
    {
        ...
        LOG_INFO("Auth client initialized successfully");
        return true;
    }
    else
    {
...
// 反初始客户端
bool CEsfClientManager::unInitializeNotifyClient()
{
    if ( __builtin_available(macOS 10.15, *) )
    {
        es_return_t ret = es_delete_client(m_pNotifyClient);
        if ( ret != ES_RETURN_SUCCESS )
        {
            LOG_ERROR("Failed to delete notify ES client, es_delete_client result={}", static_cast<int>(ret));
        }
    }
    else
    {
        LOG_ERROR("ES API not available on this macOS version for notify client");
    }
    return true;
}

bool CEsfClientManager::unInitializeAuthClient()
{
    if ( __builtin_available(macOS 10.15, *) )
    {
        es_return_t ret = es_delete_client(m_pAuthClient);
        if ( ret != ES_RETURN_SUCCESS )
        {
            LOG_ERROR("Failed to delete auth ES client, es_delete_client result={}", static_cast<int>(ret));
        }
    }
    else
    {
        LOG_ERROR("ES API not available on this macOS version for auth client");
    }
    return true;
}
...
bool CEsfClientManager::setNotifySubscription()
{
    ...
    if ( !notifyEventTypes.empty() )
    {
        ...
        for (auto eventType : notifyEvents) {
            std::string typeName;
            switch (eventType) {
                case ES_EVENT_TYPE_NOTIFY_EXIT: typeName = "NOTIFY_EXIT"; break;
                case ES_EVENT_TYPE_NOTIFY_CLOSE: typeName = "NOTIFY_CLOSE"; break;
                case ES_EVENT_TYPE_NOTIFY_FORK: typeName = "NOTIFY_FORK"; break;
                case ES_EVENT_TYPE_NOTIFY_EXEC: typeName = "NOTIFY_EXEC"; break;
