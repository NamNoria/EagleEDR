...
typedef int (*onfilecreate_fn)(const char *, const struct THREAT_PROC_INFO *, const struct FILE_CREATE_INFO *,
                               const struct BehaviorResult **);
typedef int (*onfilerename_fn)(const char *, const struct THREAT_PROC_INFO *, const struct FILE_RENAME_INFO *,
                               const struct BehaviorResult **);
typedef int (*onprocstart_fn)(const char *, const struct THREAT_PROC_INFO *, const struct THREAT_PROC_INFO *,
                              const struct BehaviorResult **);
typedef void (*freeresult_fn)(struct BehaviorResult *);

struct EAGLE_THREAT_FILE_INFO;
...
public:
    ...
    template<typename RuleType> const std::vector<RuleType> &GetFilterRule() const
    {
        initRuleOnce();
        auto it = m_ruleMap.find(std::type_index(typeid(RuleType)));
        if ( it == m_ruleMap.end() )
        {
            throw std::runtime_error("Unsupported RuleType");
        }
        return *static_cast<const std::vector<RuleType> *>(it->second);
    }
...
private:
    ...
    bool ParseConfig(const cJSON *root);
    void initRuleOnce() const;

private:
    std::string            m_configPath;  // 规则库路径
    static void           *m_libHandle;   // 规则库
    mutable std::once_flag m_initFlag;
    FilterRuleData         m_rule;

    static bool                                     m_libInited;
    static bool                                     m_configLoaded;
    mutable std::map<std::type_index, const void *> m_ruleMap;
